...
function setup_thing(thing_data) {
	...
	let last_grid_x: number
	let last_grid_y: number
	let last_valid_x: number
	let last_valid_y: number
	let old_highlighted = []

	function on_mousedown(ev: {clientX: number, clientY: number}) {
		rel_x = ev.clientX - tbl.offsetLeft
		rel_y = ev.clientY - tbl.offsetTop
		holding = true
	}

	function on_touchstart(ev: TouchEvent) {
	...
	function on_mousemove(ev: {clientX: number, clientY: number}) {
		if (holding) {
	...
	function on_touchmove(ev: TouchEvent) {
